...
CLASS /vno/2ui5_cl_core_srv_bind IMPLEMENTATION.
  ...
  METHOD main.
    ...
    DATA(lo_model) = NEW /vno/2ui5_cl_core_srv_attri( attri = mo_app->mt_attri
                                                  app   = mo_app->mo_app ).
  ...
  METHOD main_local.
    TRY.
        ...
        DATA(lo_json) = CAST /vno/2ui5_if_ajson( /vno/2ui5_cl_ajson=>new( ) ).
        lo_json->set( iv_path = `/`
                      iv_val  = val ).
        ...
        ELSE.
          lo_json = lo_json->map( /vno/2ui5_cl_ajson_mapping=>create_upper_case( ) ).
        ENDIF.

        IF config-custom_filter IS BOUND.
          lo_json = lo_json->filter( config-custom_filter ).
        ELSE.
          lo_json = lo_json->filter( /vno/2ui5_cl_ajson_filter_lib=>create_empty_filter( ) ).
        ENDIF.

        DATA(lv_id) = to_upper( /vno/2ui5_cl_util=>uuid_get_c22( ) ).
        INSERT VALUE #( name_client     = |/{ lv_id }|
                        name            = lv_id
                        json_bind_local = lo_json
                        bind_type       = /vno/2ui5_if_core_types=>cs_bind_type-one_time )
